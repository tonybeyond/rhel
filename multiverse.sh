...
# Author: AI Assistant (modified for RHEL, AlmaLinux, Rocky, and Fedora compatibility)
# Date: May 26, 2025
...
detect_os() {
    if grep -q -i "rhel" /etc/os-release; then
        ...
        OS_VERSION=$(grep -oP '(?<=VERSION_ID=")[^"]+' /etc/os-release)
        OS_MAJOR_VERSION=$(echo $OS_VERSION | cut -d. -f1)
        echo "Fedora Linux $OS_VERSION detected"
...
enable_repos() {
    ...
    if [ "$OS_TYPE" = "rhel" ]; then
        ...
        # For Fedora, no additional repos needed - everything is in main repos
        echo "Fedora detected. Using default repositories (no EPEL or CRB needed)."
    else
...
# Function to install EPEL repository (updated for RHEL, skip for Fedora)
install_epel() {
    if [ "$OS_TYPE" = "fedora" ]; then
        echo "Fedora detected. Skipping EPEL installation (not needed)."
        return 0
    fi
...
install_common_tools() {
    ...
    if [ "$OS_TYPE" = "rhel" ] && [ "$OS_MAJOR_VERSION" = "10" ]; then
        ...
        # Skip GNOME extensions installation for RHEL, we'll do it separately for Pop Shell
        echo "GNOME extensions will be installed separately."
    elif [ "$OS_TYPE" = "fedora" ]; then
        # For Fedora, most packages are available in main repos
        dnf install -y ntfs-3g libva libvdpau libva-devel libvdpau-devel neovim wget curl git btop tmux zsh neofetch fzf || echo "Some packages failed to install. Continuing..."
        
        # Install other GNOME extensions (Pop Shell will be handled separately)
        ...
        # For non-RHEL 10 systems (AlmaLinux, Rocky)
        dnf install -y ntfs-3g libva libvdpau libva-devel libvdpau-devel neovim wget curl git btop tmux zsh neofetch fzf || echo "Some packages failed to install. Continuing..."
...
install_pop_shell() {
    echo "Installing Pop Shell from source..."
    ...
    if [ -n "$ACTUAL_USER" ] && [ "$ACTUAL_USER" != "root" ]; then
        ...
        # Check out the appropriate branch based on OS version
        if [ "$OS_TYPE" = "rhel" ] || [ "$OS_TYPE" = "almalinux" ] || [ "$OS_TYPE" = "rocky" ]; then
            if [ "$OS_MAJOR_VERSION" = "8" ]; then
            ...
            # For Fedora, use the latest branch
            su - $ACTUAL_USER -c "cd $TMP_DIR && git checkout master_noble"  # Fedora usually has latest packages
        fi
...
setup_zsh() {
    ...
    if id "$ACTUAL_USER" &>/dev/null; then
        ...
        if [ -n "$ACTUAL_USER" ] && [ "$ACTUAL_USER" != "root" ]; then
            ...
            # Use different .zshrc URLs based on OS type
            if [ "$OS_TYPE" = "fedora" ]; then
                su - $ACTUAL_USER -c 'curl -fsSL -o ~/.zshrc https://raw.githubusercontent.com/tonybeyond/fedora/main/.zshrc'
            else
...
main() {
    ...
    # Install Pop Shell using distribution-specific method
    if [ "$OS_TYPE" = "fedora" ]; then
        # For Fedora, try package first, then source if needed
        echo "Installing Pop Shell on Fedora..."
        if ! dnf install -y gnome-shell-extension-pop-shell xprop; then
            echo "Package installation failed, trying from source..."
            install_pop_shell
        fi
    elif [ "$OS_TYPE" = "rhel" ]; then
        # For RHEL, always build from source
        install_pop_shell
    else
        # For AlmaLinux/Rocky, try package first, then source
    ...
    # Distribution-specific message
    if [ "$OS_TYPE" = "rhel" ] && [ "$OS_MAJOR_VERSION" = "10" ]; then
        echo "Note: This script has been configured for RHEL 10, which was just released. Some packages were installed from source."
    elif [ "$OS_TYPE" = "fedora" ]; then
        echo "Note: Fedora $OS_VERSION detected. Most packages were installed from official repositories."
    fi
